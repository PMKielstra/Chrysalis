...
def plotloglog(data_indices, data_titles, title, yaxis, orders):
    ...
    for data, data_title, marker in zip(data_streams, data_titles, markers):
        plt.loglog([Ns[i] for i in range(len(data)) if data[i] is not None], list(filter(lambda x: x is not None, data)), marker, label=data_title)
    for function, label, index in orders:
        ...
        while final is None:
            final = data[-1]
            data = data[:-1]
        plt.loglog(filtered_Ns, function(filtered_Ns) * final / function(filtered_Ns[-1]), label=label)
    plt.xlabel("N")
    plt.ylabel(yaxis)
    plt.title(title)
    plt.legend()
